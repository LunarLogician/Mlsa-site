import React from 'react';
import ProfileCard from './ProfileCard';  // Assuming you're reusing the ProfileCard component

const Management = () => {
  const teamData = [
    {
      name: 'Aden Abdullah',
      designation: 'Management Lead',
      role: 'Oversees all team operations and strategic planning.',
      linkedin: 'https://www.linkedin.com/in/sophiawilliams',
      image: 'aden.png',
    },
    {
      name: 'Jahanzaib Fareed ',
      designation: 'Management Co-Lead',
      role: 'Manages day-to-day operations and team coordination.',
      linkedin: 'https://www.linkedin.com/in/danielbrown',
      image: 'jahn.png',
    },
  ];

  return (
    <div className="min-h-screen bg-white text-white">
      {/* Header Section */}
      <header className="py-6 bg-gradient-to-r from-gray-700 via-gray-600 to-gray-500 text-center shadow-md">
        <h1 className="text-4xl font-bold text-white">Management</h1>
        <p className="text-lg text-gray-300 mt-2">
          Leading the team and ensuring smooth operations.
        </p>
      </header>

      {/* Main Content Section */}
      <main className="py-10 px-6 sm:px-8 lg:px-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {teamData.map((member, index) => (
            <ProfileCard key={index} {...member} />
          ))}
        </div>
      </main>
    </div>
  );
};

export default Management;
